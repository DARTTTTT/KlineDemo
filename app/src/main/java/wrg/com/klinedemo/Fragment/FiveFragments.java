...
import wrg.com.klinedemo.Rewrite.CustomeYAxisRenderer;
...
public class FiveFragments extends BaseFragment {
    ...
    protected void init(View view) {
        ...
        barChart.setRendererLeftYAxis(yAxisRenderer);
    ...
    public void getEventData(List<KlineEntity.DataBean> klineAlldata) {
        if (klineAlldata.get(0).getState().equals("fives")) {
            mChart.resetTracking();
            barChart.resetTracking();
            List<KlineEntity.DataBean> pastToNowData = OrderData.getPastToNowFenshiData(klineAlldata);

            //List<KlineEntity.DataBean> pastToNowFenshiData = OrderData.getPastToNowFenshiData(klineAlldata);

            candleEntriesKline = getCand.getCandleEntries(pastToNowData, 0);
            Log.d("print", "onSuccess: 处理好之后的数据" + candleEntries.toString());

            xVals = new ArrayList<>();
            for (int i = 0; i < klineAlldata.size(); i++) {
                xVals.add(pastToNowData.get(i).getTrdDt().substring(11, pastToNowData.get(0).getTrdDt().length() - 3));
            }
            ...
            axisLeftBar.setValueFormatter(new VolFormatter((int) Math.pow(10000, u)));
            axisRightBar.setValueFormatter(new VolFormatter((int) Math.pow(100000000, u)));
            //插入柱状图的数据
